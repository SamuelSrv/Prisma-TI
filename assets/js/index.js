...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Processamento do Cadastro Separado com Validações
    formCadastro.addEventListener('submit', async (e) => {
        ...
        // Validação de Senha: Mínimo 8 caracteres, contendo letras e números
        const regexSenha = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;
        if (!regexSenha.test(password)) {
            const errEl = document.getElementById('error-password');
            errEl.textContent = 'A senha deve ter no mínimo 8 caracteres, com letras e números.';
        ...
        try {
            // 1. Validação prévia: Verifica se o CPF já existe na tabela de perfis
            ...
            // 2. Registro no Supabase Auth com metadados para o gatilho
